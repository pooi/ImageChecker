...
from NoHierarchicalAccuracy import Checker
...
def collectImage(dir_name, data, summary, check_rates):
    # print("Dir: ", dir_name)
    list = []
    for dir in os.listdir(dir_name):
        if dir.startswith("."):
        ...
        path = os.path.join(dir_name, dir)
        if os.path.isdir(path):
            collectImage(path, data, summary, check_rates)
        else:
            fname, ext = os.path.splitext(path)
            ext = ext.lower()
            if ext.endswith("jpeg") or ext.endswith("jpg") or ext.endswith("png") or ext.endswith("gif"):
...
def printSummary(summary, check_rates):
    ...
    num_of_divider = len(check_rates)*8 + 29 + 10
...
if __name__ == "__main__":
    ...
    try:
        data = {}
        summary = {}
        collectImage(dirname, data, summary, check_rates)
        ...
        for step in range((length // DIV) + 1):
            ...
            for i in range(num_of_thread):
                checker = Checker(i, queue, sess, output_operation, input_operation, labels, check_rates, base_accuracy,
            ...
            sess.__del__()
            print("Step", (step+1))
            printSummary(summary, check_rates)
